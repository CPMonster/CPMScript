...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:function name="cpm:parse">
        ...
        <xsl:variable name="seqTargets" as="xs:string*">
            <xsl:copy-of select="cpm:rhist.targets($elmNewHistory)"/>
        </xsl:variable>

        <xsl:if test="count($seqTargets)">

            <xsl:variable name="strNextId" select="$seqTargets[1]"/>
            ...
            <xsl:if test="exists(root($elmItem)//node[@id = $strNextId])">
                <xsl:copy-of
                    select="cpm:parse(root($elmItem)//node[@id = $strNextId], $elmNewHistory)"/>
            </xsl:if>
